    ...
    public void testWebServiceContextInject(@ArquillianResource URL deploymentUrl) throws Exception {
        ...
        try {
            ...
            for (Future<String> result : results) {
                Assert.assertTrue(result.get(5, TimeUnit.SECONDS).startsWith("Hello"));
            }
        } finally {
            executor.shutdown();
        }
    ...
    private class Worker implements Callable<String> {
        ...
        public String call() {
            ...
            try {
                ...
                Assert.assertFalse("There was an error injecting javax.xml.ws.WebServiceContext. " +
                    "This might be caused by https://issues.jboss.org/browse/WFLY-9884",
                    t.getMessage().contains("WFLYWELD0049"));
                throw t;
            }
            return result;
        }
